...
func initDbPoolConfig() {
	// 数据库配置map
	dBMap := make(map[string]string)
	// 格式化数据库链接
	NewImConnect := "root:123456@tcp(127.0.0.1:3307)/demo?charset=utf8mb4"
	dBMap["demo"] = NewImConnect
	DBConfig = dBMap
}
...
func (p *DBPool) MyFilterKeyEqValue(params map[string]interface{}) *DBPool {
	where_sql := ""
	if p.whereConditionString == "" {
		where_sql = " 1=1 "
	}
	for field, value := range params {
		where_sql += fmt.Sprintf(" AND `%s`='%s'", field, value)
	}
	if p.whereConditionString == "" {
		p.whereConditionString = where_sql
	} else {
		p.whereConditionString += where_sql
	}
	return p
...
func (p *DBPool) MyFilterKeyLikeValue(params map[string]interface{}) *DBPool {
	where_sql := ""
	if p.whereConditionString == "" {
		where_sql = " 1=1 "
	}
	for field, value := range params {
		where_sql += fmt.Sprintf(" AND `%s` like '%%%s%%'", field, value)
	}
	if p.whereConditionString == "" {
		p.whereConditionString = where_sql
	} else {
...
func (p *DBPool) SelectField(field string) *DBPool {
	p.selectField = field
	return p
}
...
//CheckTableIsExists 检查表是否存在
func (p *DBPool) CheckTableIsExists(dbname string) bool {
	CheckTableSql := fmt.Sprintf("SELECT `TABLE_NAME` FROM`INFORMATION_SCHEMA`.`TABLES` WHERE `TABLE_SCHEMA` = '%v'  AND `TABLE_NAME` = '%v'", dbname, p.tableName)
	row := p.FetchOne(CheckTableSql)
...
func (p *DBPool) MyGetOne() map[string]interface{} {
	GetOneSql := fmt.Sprintf("SELECT %v FROM %v WHERE %v", p.selectField, p.tableName, p.whereConditionString)
	row := p.FetchOne(GetOneSql)
	return row
}

func (p *DBPool) MyGetAll() []map[string]interface{} {
	GetAllSql := fmt.Sprintf("SELECT %v FROM %v WHERE %v", p.selectField, p.tableName, p.whereConditionString)
	rows, err := p.pool.Query(GetAllSql)
	CheckError(err, 0)
	// 数据获取
	RetMap := dealMysqlRows(rows)
	return RetMap
...
func (p *DBPool) Update(params map[string]interface{}) (affectRows int, err error) {
	...
	// 组合数据更新SQL
	UpdateSql := fmt.Sprintf("UPDATE %v SET %v WHERE %v;", p.tableName, UpdateArgs, p.whereConditionString)
...
func (p *DBPool) Delete() (affectRows int, err error) {
	// 组合删除数据SQL
	DeleteSql := fmt.Sprintf("DELETE FROM %v WHERE %v", p.tableName, p.whereConditionString)
